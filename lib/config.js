...
function normalizeBarkDeviceKey(value) {
  const raw = typeof value === "string" ? value.trim() : "";
  if (!raw) {
    return "";
  }

  return raw.replace(/^https?:\/\/api\.day\.app\//i, "").split(/[/?#]/)[0].trim();
}

function getConfig() {
  const barkBaseUrl = process.env.BARK_BASE_URL || "https://api.day.app";
  const barkDeviceKey = normalizeBarkDeviceKey(process.env.BARK_DEVICE_KEY || "");
  ...
  return {
    ...
    barkVolume: process.env.BARK_VOLUME || "10",
