...
async function sendBarkPayload(config, payload) {
...
async function sendBarkAlert(config, details) {
  ...
  return sendBarkPayload(config, payload);
}

async function sendTestBark() {
  const config = getConfig();
  const payload = {
    title: `Fluid ${config.alertSymbol} Bark 测试`,
    body: [
      "这是一条测试通知。",
      `监控币种: ${config.alertSymbol}`,
      `阈值: ${formatCompactUsd(config.alertLiquidityThresholdUsd)}`,
